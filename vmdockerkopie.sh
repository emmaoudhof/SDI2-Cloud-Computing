...
for ((i=0; i<vm_aantal; i++)); do
    ...
    ssh ${plek_node} "qm reset ${vmid_nieuw}"
    echo "dockergroup en user vm: ${vmid_nieuw}, wacht nog even 200 sec aub!"
    sleep 200

    ssh -i ${sshkey_padopslag} emma@${ip_nieuw} "cd SDI2-Cloud-Computing && sudo bash imagedocker.sh"
    ssh -i ${sshkey_padopslag} emma@${ip_nieuw} "cd SDI2-Cloud-Computing && sudo bash composedocker.sh"
    ssh -i ${sshkey_padopslag} emma@${ip_nieuw} "cd SDI2-Cloud-Computing && sudo bash swarmaanmaak.sh"
